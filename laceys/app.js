...
DEFAULT_DOCKS.forEach(d=>{const g=walkGeomFromDock(d);DEFAULT_MARKS.push({id:"walk-"+d.id,kind:"bar",x:g.x,y:g.y,w:g.w,h:g.h,title:"Walk "+d.name});DEFAULT_MARKS.push({id:"dlabel-"+d.id,kind:"text",x:d.x,y:d.y-12,text:d.name,size:14});});
function loadLayout(){
  try{
    ...
    const mBy=Object.fromEntries((raw.marks||[]).map(m=>[m.id,m]));
    const marks=DEFAULT_MARKS.map(m=>Object.assign(clone(m), mBy[m.id]||{}));
    (raw.marks||[]).forEach(m=>{ if(!marks.find(x=>x.id===m.id)) marks.push(m); });
    return {docks,marks};
...
function redraw(){
  ...
  const byDock={};
  docks.forEach(d=>{const rot=Number(d.rot)||0;const attrs={"data-dock":d.id,class:"dock-hit"+(selectedDock===d.id?" on":"")};if(rot)attrs.transform=`rotate(${rot} ${d.x} ${d.y})`;const g=el("g",attrs);byDock[d.id]=g;layerDocks.appendChild(g);});
...
document.getElementById("add-walk").onclick=()=>{const m={id:uid("walk"),kind:"bar",x:200,y:200,w:14,h:220,title:"Walkway",rot:0};marks.push(m);selectedMark=m.id;selectedDock=null;saveLayout();showTab("layout");redraw();renderDockEditor();};
