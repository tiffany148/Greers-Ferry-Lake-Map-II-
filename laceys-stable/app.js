...
function loadLayout(){
  try{
    const raw=JSON.parse(localStorage.getItem(LAYOUT_STORE)||localStorage.getItem("laceys-layout-v1")||"null");
...
function buildSlips(){
  ...
  docks.forEach(d=>{
    generatedSlips(d).forEach(s=>{
      ...
      slips.push({id:p.id,num:p.num,dock:d.name,dockId:d.id,kind:p.kind||d.kind,size:p.size||d.size,x:p.x,y:p.y,w:p.w,h:p.h,rot:p.rot||0,filter:p.filter||d.name});
    });
  });
}
...
function renderDockEditor(){
  ...
  if(s && editing && selectedDock && d && !isLocked(d)){
    box.innerHTML=`<h2>Slip ${s.num}</h2><p class="hint">Unlocked · drag this slip on the chart</p><div class="row2"><label>X<input id="ed-x" type="number" value="${Math.round(s.x)}"/></label><label>Y<input id="ed-y" type="number" value="${Math.round(s.y)}"/></label></div><div class="row2"><label>Width<input id="ed-w" type="number" value="${Math.round(s.w)}"/></label><label>Height<input id="ed-h" type="number" value="${Math.round(s.h)}"/></label></div>${rotCtrl(Number(s.rot)||0)}<div class="st"><button type="button" data-nudge="-10,0">←</button><button type="button" data-nudge="10,0">→</button><button type="button" data-nudge="0,-10">↑</button><button type="button" data-nudge="0,10">↓</button></div><div class="st"><button type="button" id="ed-del-slip">Delete this slip</button></div>`;
    const apply=()=>{setPlaced(d,s.id,{x:+document.getElementById("ed-x").value,y:+document.getElementById("ed-y").value,w:+document.getElementById("ed-w").value,h:+document.getElementById("ed-h").value});saveLayout();redraw();};
  ...
  if(dockDrag){
    ...
    if(dockDrag.kind==="slip"){
      ...
      if(d){
        ...
        if(isLocked(d)) moveDockSlips(d,nx-d.x,ny-d.y);
        d.x=nx; d.y=ny; redraw();
      }
    }else{
      const m=marks.find(x=>x.id===dockDrag.id);
      if(m){ m.x=Math.round(dockDrag.x+dx); m.y=Math.round(dockDrag.y+dy); redraw(); }
    }
    return;
  }
  if(!pan) return;
