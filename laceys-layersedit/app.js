...
const PHOTO_ALIGN_STORE="laceys-layersedit-photo-align-v1";
...
const LABEL_SIZE_STORE="laceys-layersedit-label-size-v1";
...
function loadLayersStandalone(){
  try{
    const raw=JSON.parse(localStorage.getItem("laceys-layersedit-layers-v1")||"null");
...
function restoreSnap(s){
  ...
  try{ localStorage.setItem("laceys-layersedit-layers-v1", JSON.stringify(layers)); }catch(e){}
...
if(_fixBlank) _fixBlank.onclick=()=>{
  ...
  try{ localStorage.removeItem(LAYOUT_STORE); /* layersedit isolated */ /* layersedit isolated */ }catch(e){}
