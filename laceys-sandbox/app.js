...
const PHOTO_ALIGN_STORE="laceys-sandbox-photo-align-v87";
...
function loadLayersStandalone(){
  try{
    const raw=JSON.parse(localStorage.getItem("laceys-sandbox-layers-v1")||"null");
...
function restoreSnap(s){
  ...
  try{ localStorage.setItem("laceys-sandbox-layers-v1", JSON.stringify(layers)); }catch(e){}
...
if(_fixBlank) _fixBlank.onclick=()=>{
  ...
  multi.clear(); moveWholeChart=false; hist.length=0; future.length=0; lastSnap=snap(); saveLayout(false); ensureMapVisible(); renderDockEditor(); renderChips(); updateUndoBtns(); alert("Sandbox reset to blank.");
};
